...
#include <tuple>
#include <vector>
#include <regex> // Added to utilize the regex tooling
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Solution ============================================================================
  // The following creates a regex expression to look forward a specific attack vector to
  // defend against. Such as the part of the attack equal to " or ". If it finds such a
  // thing inside of the sql query it will assume its an injection attempt and return
  // false for the method thus failing the query.
  // This is the overall wrong way to go about combatinng SQL injection and should be done
  // via SQL prepared statements to parameterize a query.
  std::regex str_exp("( or )");
  if (std::regex_search(sql, str_exp))
  {
      std::cerr << "Suspected SQL injection detected!!" << std::endl;
      std::cout << "Bad sql: " << sql << std::endl;
      return false;
  }
  // Solution ============================================================================
...
{
  std::cout << "SQL: " << sql << " ==> " << records.size() << " records found." << std::endl;
...
void run_queries(sqlite3* db)
{
  ...
  // Added to figure out which sql queries are injections
  std::cout << "==================Running injected lines=====================" << std::endl;
