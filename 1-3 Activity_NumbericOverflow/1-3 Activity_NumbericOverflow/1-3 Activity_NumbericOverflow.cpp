...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        // The following if statement checks to see if the result ever becomes larger
        // than the T datatypes max value. If it does it will just print out an error
        // letting the user know that something is a muck.
        // - Detects when an overflow will happen
        // - Informs the user that the overflow happened
        // - Still returns the same result before the method was changed
        if (increment > 0 && result > std::numeric_limits<T>::max() - increment) {
            std::cerr << "Overflow detected in add_numbers()! Result-> ";
        }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // The following if statement checks to see if the result ever becomes smaller
        // than the T datatypes min value. If it does it will just print out an error
        // letting the user know that something is a muck.
        // - Detects when an underflow will happen
        // - Informs the user that the underflow happened
        // - Still returns the same result before the method was changed
        if (decrement > 0 && result < std::numeric_limits<T>::min() + decrement) {
            std::cerr << "Underflow detected in subtract_number()! Result -> ";
        }
...
void test_underflow()
{
    ...
    //
    //  There are more than one possible solution to this problem. 
    //  The solution must work for all of the data types used to call test_underflow() in main(). // Edited from test_overflow() to test_underflow() to follow the correct method names
    ...
    std::cout << "\tSubtracting Numbers Without Underflow (" << +start << ", " << +decrement << ", " << steps << ") = "; // Edited from "Without Overflow" to "Without Underflow" to follow the correct semantics
    auto result = subtract_numbers<T>(start, decrement, steps);
    std::cout << +result << std::endl;

    std::cout << "\tSubtracting Numbers With Underflow (" << +start << ", " << +decrement << ", " << (steps + 1) << ") = "; // Edited from "Without Overflow" to "Without Underflow" to follow the correct semantics
